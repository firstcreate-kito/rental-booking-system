...
import { applyRefundFee, type RefundFee } from '../lib/refund-fee';
...
app.post('/lookup', async (c) => {
  ...
  let cancelQuote: { cancelFee: number; refundAmount: number; chargePctMax: number; paidAmount: number; refundFee: RefundFee } | null = null;
  if (contact.status !== 'cancelled') {
    ...
    if (grp) {
      ...
      cancelQuote = { cancelFee: q.cancelFee, refundAmount: rf.net, chargePctMax: q.chargePctMax, paidAmount: q.paidAmount, refundFee: rf };
    }
  }
  return c.json({
    matched: true,
...
app.post('/request', async (c) => {
  ...
  if (type === 'cancel') {
    ...
    if (grp) {
      ...
      refundAmount = applyRefundFee(q.refundAmount, grp.payment_method).net;
    }
  }
