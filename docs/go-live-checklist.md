...
7. [ ] **移行顧客の会員化＋紐付け（案A）**：本番管理画面→「移行顧客の会員化・案内」→①プレビュー→②会員化＆紐付け。回数券残も同じメールで同一会員に自動連携。**必ず 5・6 の後に**。🧑‍💻
8. [ ] **目視GO/NO-GO**：既知の予約数件（例：赤井様 9/10 13:00）がカレンダー・空き状況・管理画面に正しく出るか。サイネージ本文（タイトルの平日/土日祝・`[スペース名]`ブロック）も確認。🧑‍💻
9. [ ] **予約システムのゲート解除（＝予約システムを先に公開）＋予約スモークテスト**：本番Workerの `BASIC_AUTH_USER` と `BASIC_AUTH_PASS` を削除（Cloudflareダッシュボード → Worker → Settings → Variables and Secrets、または `wrangler secret delete`）。両方消えるとゲートOFF。**これで案内メールのログインリンクが機能する**。テスト予約1件で決済・Google予定・メール・Webhook・領収書を確認。🧑‍💻
10. [ ] **★移行案内メールを送信（45人）**：本番管理画面→「移行顧客の会員化・案内」→**宛先確認→送信**（本番でのみ実送信）。**公式サイトを開ける前・公開ラッシュ前**に送ることで、ログインの問い合わせに落ち着いて対応できる。**送信は撤回不可**。🧑‍💻
11. [ ] **公式サイトの申込導線を新システムへ切替＋トップのカーテンを開けて一般公開**：`.htaccess`（メンテ告知削除＋`DirectoryIndex`）。予約ボタン→ booking.space-albe.com、お問い合わせは既に `/api/contact` 連携済み。ConoHaキャッシュ削除。🧑‍💻
12. [ ] **Bookly はアンインストールせず「無効化のみ」で猶予保持**（旧予定がGoogleに残ることを確認、未来のBookly予約が消化されてから撤去検討）。🧑‍💻
