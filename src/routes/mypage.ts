...
app.get('/bookings/:number/cancel-quote', async (c) => {
  ...
  const formula = cancelFormulaLines({
    ...
    ticket,
    refundFee: rf.applied ? rf : undefined,
  });
  // 支払方法も返す（'invoice'＝自社口座への直接振込のみ、返金時に振込手数料の注記を表示）。
  // お客様提示のご返金額(refundAmount)は決済手数料控除後(net)。gross・手数料は refundFee に同梱。
  return c.json({ ...q, refundAmount: rf.net, refundFee: rf, paymentMethod: g.payment_method, ticket, formula });
});
...
app.get('/bookings/:number/reschedule-quote', async (c) => {
  ...
  return c.json({ ...quote, settlement, refundFee: rf, formula });
});
...
app.post('/bookings/:number/reschedule', async (c) => {
  ...
  let refundFeeApplied: RefundFee | undefined;
  let kind: string;
  let note: string;
  if (exec.ticket.isTicket) {
    ...
    if (s.kind === 'increase' || s.kind === 'cancel_treatment') {
      ...
      const rf = applyRefundFee(grossRefund, g.payment_method);
      quotedAmount = rf.net;
      refundFeeApplied = rf.applied ? rf : undefined;
    } else {
      direction = 'none';
      quotedAmount = 0;
    }
