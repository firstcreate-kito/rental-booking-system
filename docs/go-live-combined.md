...
7. [ ] **移行顧客の会員化＋紐付け（案A）**：管理画面→システム状態→「移行顧客の会員化・案内」→ ①プレビュー → ②会員化＆紐付け。8/30以降の予約を持つ45人を会員化し予約に紐付け、回数券残（#82）も同じメールで同一会員に自動連携。**必ず 5（取り込み）と 6（チケット）の後**に実行。案内メールは公開後（ゲート解除後）に「宛先確認→送信」。
